...
import numpy as np
import json
from . import Q3_open_source_image_processing_functions as img_proc
from . import Q3_open_source_fit_two_half_gaussians as function_fitting
...
def batch_analyze_npz_folder(folder_path, slice_index=0, fit_function=None, pattern="*.npz"):
    """
    Analyze all NPZ files in a folder and print results in a neat format.
    ...
    slice_index : int, optional
        Index of the slice/ROI to analyze in each file (default: 0)
    fit_function : callable, optional
        Fit function to use. Defaults to fit_composite_PREFFERRED_MODEL_ATM_09102024.
    ...
    npz_files = sorted(folder.glob(pattern))
    ...
    print(f"\n{'='*80}")
    print(f"Analyzing {len(npz_files)} NPZ files from: {folder_path}")
    print(f"{'='*80}\n")
    print(f"{'Filename':<50} {'CNR':>8} {'Step σ':>10} {'Comp σ':>10}")
    print(f"{'-'*80}")
    
    results = []
    for npz_file in npz_files:
        try:
            result = slice_quick_IQM_npz(
                npz_path=npz_file,
            ...
            print(f"{filename:<50} {cnr:>8.3f} {step_sig:>10.4f} {comp_sig:>10.4f}")
            ...
            results.append({
                ...
                'CNR': cnr,
                'step_sigma': step_sig,
                'composite_sigma': comp_sig,
                'metadata': result.get('metadata')
            })
            
        except Exception as e:
            print(f"{npz_file.name:<50} ERROR: {str(e)}")
