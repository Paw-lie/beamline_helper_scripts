...
def get_beamline_profile(beamline):
    ...
    if not profile:
        print(f"⚠️  No configuration found for beamline '{beamline}'")
        return {}
    return profile
...
def init_settings(existing_settings):
    ...
    if new_settings['beamline'] == "ESRF":
        try:
            ...
            print("⚠️  Invalid camera size. Setting to NaN.")
...
def scan_parameters(settings):
    ...
    if not processed_root or not os.path.isdir(processed_root):
        print("⚠️  Invalid processed data path.")
        return
    ...
    for scan_id in scan_ids:
        ...
        if not files:
            print(f"\n⚠️  No matching files in scan {scan_id}")
            continue
...
def check_reconstruction_status(settings):
    ...
    if not os.path.isdir(raw_root) or not os.path.isdir(processed_root):
        print("⚠️  Both paths must be valid directories.")
        return
...
def check_reconstruction_details(settings):
    ...
    For each raw scan folder:
        - Check if processed folder exists
        - Check if any *.vol file exists with size > 0
        - If exists, parse parameter files and map them
        - Print key params (Paganin, unsharp_coeff, unsharp_sigma, rotation_axis)
        - Determine MODE (FT/HA/ERROR/NaN)
    """
    import math
    ...
    # Ask for camera size if ESRF and not present in settings
    camera_size = None
    if beamline == "ESRF":
        if "camera_size_x_y" not in settings or settings["camera_size_x_y"] is None:
            try:
                cam_size = input("Enter camera size (number of pixels, e.g. 2048): ").strip()
                camera_size = float(cam_size)
                settings["camera_size_x_y"] = camera_size
            except Exception:
                print("⚠️  Invalid camera size. Setting to NaN.")
                camera_size = float('nan')
                settings["camera_size_x_y"] = camera_size
        else:
            camera_size = settings["camera_size_x_y"]
    ...
    header = (
        f"{'SCAN_ID':<100} {'EXISTS':<7} {'.VOL':<7} "
        f"{'PAGANIN':<10} {'UNSH_COEFF':<12} {'UNSH_SIGMA':<12} "
        f"{'AXIS_POS':<10} {'MODE':<5}"
    )
    print(header)
    print("-" * len(header))
    ...
    for scan in raw_scans:
        ...
        if exists:
            ...
            if combined_data:
                ...
                # For MODE
                vol_size_x = safe_float(params.get('vol_size_x'))
                vol_size_y = safe_float(params.get('vol_size_y'))

                # Use camera size from params or settings
                if beamline == "ESRF":
                    cam_size = camera_size
                else:
                    cam_size = safe_float(params.get('camera_size_x_y'))

                if math.isnan(vol_size_x) or math.isnan(vol_size_y) or math.isnan(cam_size):
                    mode = "NaN"
                else:
                    ratio_x = vol_size_x / cam_size
                    ratio_y = vol_size_y / cam_size
                    if abs(ratio_x - 1) <= 0.02 and abs(ratio_y - 1) <= 0.02:
                        mode = "FT"
                    elif ratio_x > 1.02 or ratio_y > 1.02:
                        mode = "HA"
                    else:
                        mode = "ERROR"
        ...
        print(
            f"{scan:<100} {exists_mark:<7} {vol_mark:<7} "
            f"{paganin:<10} {unsharp_coeff:<12} {unsharp_sigma:<12} {axis_display:<10} {mode:<5}"
        )
...
def show_pending_details(settings):
    ...
    - 🟡 for ongoing (<5min) or incomplete .vol files (with reason)
    - 🔴 for stalled (>5min)
    - ❓ for unknown (no files found)
    - 🟢 for done (.vol exists, >0GB, z-size matches camera z)
    ...
    print("  🟡  ongoing or incomplete .vol file. Size or file-size does not match expectations.")
    print("  🔴  stalled (>5min)")
    print("  ❓  unknown (no files found)")
    print("  🟢  done (.vol exists, >0GB, z-size matches camera z)")
...
def show_scan_details(settings, identifier):
    ...
    try:
        ...
        print(f"⚠️  Error listing processed data path: {e}")
        return

    if not scan_ids:
        print(f"⚠️  No scan folders found containing '{identifier}'.")
        return
...
def create_scan_views(settings):
    ...
    if not views_folder or not os.path.isdir(views_folder):
        print("⚠️  Invalid views folder. Please run 'init_views' first.")
        return
        ...
        for vol_file in vol_files:
            ...
            for candidate in os.listdir(info_dir):
                ...
                if any(candidate.endswith(ext) for ext in exts):
                    ...
                    break
            if not info_file:
                print(f"⚠️  No info file found for {vol_file}, skipping.")
                continue
            ...
            try:
                ...
                print(f"⚠️  Could not determine shape for {vol_file}, skipping.")
                continue
            ...
            try:
                ...
                print(f"⚠️  Error creating view for {vol_file}: {e}")
...
def create_ortho_slices(settings, identifier=None):
    ...
    if not views_folder or not os.path.isdir(views_folder):
        print("⚠️  Invalid or missing views folder. Please select a folder.")
        settings = init_views_folder(settings)
        views_folder = settings.get("views_folder", "")
        if not views_folder or not os.path.isdir(views_folder):
            print("⚠️  Views folder still invalid. Aborting.")
            return
    ...
    if not scan_ids:
        print("⚠️  No scan folders found." + (f" (filtered by '{identifier}')" if identifier else ""))
        return
    ...
    try:
        for scan_id in scan_ids:
            ...
            for vol_file in vol_files:
                ...
                if not os.path.isfile(info_file):
                    print(f"⚠️  No info file for {vol_file}, skipping.")
                    continue
        ...
        print("\n⏹️  Ortho slice creation interrupted by user. Returning to command prompt.")
        return
...
def main():
    ...
    while True:
        try:
            ...
            try:
                confirm = input("\n⚠️  Do you really want to exit? (y/N): ").strip().lower()
            except EOFError:
                print('\nGoodbye!')
                break
            if confirm == 'y':
